                            ...
                            TextInput::make('tiktok')
                                ...
                                ->rules([
                                    fn (): \Closure => function (string $attribute, $value, \Closure $fail) {
                                        ...
                                        if (filter_var($value, FILTER_VALIDATE_URL) || str_contains($value, 'tiktok.com')) {
                                            if (! preg_match('/^(https?:\/\/)?(www\.)?tiktok\.com\/@[a-zA-Z0-9._]{2,24}\/?$/i', $value)) {
                                                $fail('The TikTok URL is not valid.');
                                            }
                                        } else {
                                            $username = ltrim($value, '@');
                                            if (! str_starts_with($value, '@') || ! preg_match('/^[a-zA-Z0-9._]{2,24}$/', $username) || str_ends_with($username, '.')) {
                                                $fail('The TikTok username must start with @ and contain 2 to 24 letters, numbers, periods, or underscores.');
                                            }
                                        }
                                    },
                                ]),
